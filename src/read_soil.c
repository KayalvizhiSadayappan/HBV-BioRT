...
void ReadSoil(const char dir[], int nsub, subcatch_struct sub[])
{
    ...
    biort_printf(VL_NORMAL, "  Surface max total water storage is %.2f mm.\n", sub[0].d_surface);

    NextLine(fp, cmdstr, &lno);
    ReadParam(cmdstr, "WS_MAX_UZ", 'd', fn, lno, &sub[0].d_uz);
    biort_printf(VL_NORMAL, "  Upper zone max total water storage is %.2f mm.\n", sub[0].d_uz);

    NextLine(fp, cmdstr, &lno);
    ReadParam(cmdstr, "WS_MAX_LZ", 'd', fn, lno, &sub[0].d_lz);
    biort_printf(VL_NORMAL, "  Lower zone max total water storage is %.2f mm.\n", sub[0].d_lz);
